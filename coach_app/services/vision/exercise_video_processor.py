...
class VideoProcessorClass(VideoProcessorBase):
    def __init__(self):
        ...
        self._detectors = {
            ...
            # Chest — push-up variants share identical joint-angle logic;
            # hand/foot placement isn't distinguishable from a single camera.
            ...
            # Biceps — curl variants share identical logic (equipment doesn't
            # change joint kinematics a camera can see).
    ...
    def _draw_overlays(self, img, metrics, ex_type):
        if ex_type == "Squats":
            ...
            self._draw_generic_overlays(img, metrics)

    def _draw_generic_overlays(self, img, metrics):
        """Fallback overlay for the Exercise Library additions — prints
        whichever status field the detector returned instead of a numeric
        angle, so every new exercise gets an on-screen readout without
        needing its own dedicated overlay method."""
        h, _ = img.shape[:2]

        status_value = None
        for key, value in metrics.items():
            if key in ("reps", "pose_detected"):
                continue
