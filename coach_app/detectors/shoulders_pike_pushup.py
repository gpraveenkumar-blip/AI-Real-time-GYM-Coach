...
class PikePushUpDetector(BaseExercise):
    """Push-up with hips raised into an inverted-V (pike) shape, shifting
    load onto the shoulders. Tracks elbow flexion for reps like a regular
    push-up, plus a hip-angle check that the pike shape is being held.
    """
    DOWN_THRESHOLD = 90
    UP_THRESHOLD = 160
    MIN_VISIBILITY = 0.7
    PIKE_MAX_ANGLE = 100   # hip angle should stay acute (piked), not flat
    ...
    def process(self, landmarks) -> dict:
        ...
        pike_status = "PIKED" if hip_angle <= self.PIKE_MAX_ANGLE else "RAISE HIPS HIGHER"

        return {
            "reps": self.reps,
            "elbow_angle": int(elbow_angle),
            "pike_status": pike_status,
        }
